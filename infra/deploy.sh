...
docker save "shtil/api:$VERSION" "shtil/collector:$VERSION" "shtil/worker:$VERSION" "shtil/migrate:$VERSION" "shtil/seed:$VERSION" "shtil/lawtool:$VERSION" | gzip | ssh "$HOST" 'gunzip | docker load' | sed 's/^/   /'
