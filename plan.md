...
- [~] Интерфейс `LLM` и клиент GigaChat готовы; `Embedder` отложен: эмбеддинги на бесплатном тарифе недоступны (HTTP 402), склейка идёт лексически (revisit.md №35).
- [x] Склейка по центроидам (TF-IDF вместо эмбеддингов); порог 0.30 подобран на тестовом корпусе, отчёт `worker debug-clusters` есть. Подобрать на реальных данных — после подключения проверенных источников.
- [x] Классификация и саммари со строгим JSON (вызов функции GigaChat), валидацией и повтором, дебаунс 10 минут.
- [~] Метрики: итог каждого прохода в логе (постов, сюжетов, токенов, публикаций); отдельной системы метрик нет.

### Фаза 5 — Юридический блок (4–6 дней)
- [x] Исследовать API и форматы: список законов — открытый JSON API publication.pravo.gov.ru; PDF там сканы без текста, полный текст берём с kremlin.ru; api.duma.gov.ru недоступен, СОЗД (законопроекты) отложен.
- [x] Отбор «касается граждан/ИП», извлечение структуры и тегов аудитории (`lawtool fetch`).
- [x] `lawtool review` для ручной проверки; публикуются только проверенные (проверяет человек, не я).
