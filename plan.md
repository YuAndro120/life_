...
- [x] Экраны: Today, LawDetail, Calendar, Filters, кастомный таб-бар (плюс вкладка «Профиль»).
- [x] Онбординг: Welcome → Profile → Calm → Theme (живая перекраска) → Building (анимация на реальных цифрах).
...
- [~] `seeds/sources.yaml`: 13 кандидатов (2 государственных включены, 11 ждут проверки по реестрам; @NeuralProfit и @simply_formula в списке). До 20–40 доберём вместе с владельцем проекта, см. revisit.md №29.
- [x] Коллекторы RSS/Atom и `t.me/s`, расписание, бэкофф при ошибках, пауза между запросами к одному хосту, User-Agent.
- [x] Фильтр рекламы по маркировке (erid, #реклама, «реклама + ИНН»); спорные случаи помечаются для LLM (фаза 4).
