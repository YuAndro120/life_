...
/// Адрес API: из Info.plist (`LifeAPIBaseURL`, build setting `API_BASE_URL`), в DEBUG можно переопределить `-lifeAPI <url>`.
enum AppConfig {
    static var apiBaseURL: URL? {
        if let override = DebugLaunch.apiOverride, let url = URL(string: override) { return url }
        guard let raw = Bundle.main.object(forInfoDictionaryKey: "LifeAPIBaseURL") as? String else { return nil }
