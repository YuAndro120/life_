...
struct FilterPreferences: Equatable, Codable, Sendable {
    ...
    /// Скрывать новости других регионов: остаются федеральные и свой регион (из профиля). По умолчанию включено.
    var hideOtherRegions: Bool = true
    /// Регион пользователя; подставляется из профиля при сборке выпуска и не хранится в настройках.
