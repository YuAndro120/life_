...
7. audience_tags: ОБЯЗАТЕЛЬНОЕ поле, строка с 1–4 тегами через запятую, только из списка: ` + strings.Join(llm.AudienceTags, ", ") + `. Пример: "work:ip_usn, work:employee". "all", если касается всех граждан. Пустым быть не может. Теги industry:* ставь, если закон касается работников или предприятий отрасли; sells:* — если закон задаёт правила продажи конкретных товаров или услуг (например, маркировка, алкоголь, маркетплейсы). Узкий закон не помечай тегом all. Тег work:ip, work:employee и т. п. ставь, только если закон касается ВСЕХ таких работников или предпринимателей; если закон про конкретную отрасль (рыболовство, сельское хозяйство, перевозки, общепит и т. д.), ставь industry:* или sells:* вместо общего work:*.
